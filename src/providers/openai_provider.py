...
class OpenAIProvider(BaseProvider):
    """OpenAI provider for chat and image generation."""
    ...
    async def draw(self, prompt: str, model: str, **kwargs) -> ProviderResponse:
        ...
        if not self.supports_capability(Capability.DRAW):
            return ProviderResponse(
                success=False,
                error="Draw capability not supported",
                provider=self.name,
                model=model
            )
    ...
    async def speak(self, text: str, model: str, voice: str = None, **kwargs) -> ProviderResponse:
        """OpenAI doesn't support TTS in this implementation."""
        return ProviderResponse(
            success=False,
            error="Speak capability not supported by OpenAI provider",
            provider=self.name,
            model=model
        )
